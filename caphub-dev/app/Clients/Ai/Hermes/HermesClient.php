...
class HermesClient
{
    ...
    protected function baseUrl(): string
    {
        ...
        if ($baseUrl === '') {
            throw new RuntimeException('Hermes base URL is not configured.');
        }
    ...
    protected function apiKey(): string
    {
        ...
        if ($apiKey === '') {
            throw new RuntimeException('Hermes API key is not configured.');
        }
    ...
    {
        return trim((string) config('services.hermes.profile', 'chemical-news-translator'));
    }

    protected function providerModel(): string
    {
        return trim((string) config('services.hermes.model', 'gpt-5-mini'));
    }
    ...
    protected function validateTranslationResponse(
        ...
        if (! is_array($response)) {
            throw new RuntimeException('Hermes response payload must be a JSON object.');
        }

        foreach (['translated_document', 'glossary_hits', 'risk_flags', 'notes', 'meta'] as $key) {
            if (! array_key_exists($key, $response)) {
                throw new RuntimeException(sprintf('Hermes response payload is missing required key [%s].', $key));
            }
        }
        ...
        if (! is_array($response['translated_document'])
            ...
            throw new RuntimeException('Hermes response payload has an invalid schema.');
        }

        if (! array_key_exists('schema_version', $response['meta'])) {
            throw new RuntimeException('Hermes response meta is missing [schema_version].');
        }
        ...
        foreach (array_keys((array) ($payload['input_document'] ?? [])) as $key) {
            if (! array_key_exists($key, $response['translated_document'])) {
                ...
                throw new RuntimeException(sprintf('Hermes translated_document is missing required key [%s].', $key));
            }

            if (! is_string($response['translated_document'][$key])) {
                throw new RuntimeException(sprintf('Hermes translated_document key [%s] must be a string.', $key));
            }
        }
    ...
    protected function normalizeResponsePayload(mixed $response): array
    {
        ...
        if (! is_array($decoded)) {
            throw new RuntimeException('Hermes completion content must be a JSON object.');
        }

        $translatedDocument = data_get($decoded, 'translated_document');
        if (! is_array($translatedDocument)) {
            throw new RuntimeException('Hermes completion content is missing [translated_document].');
        }
    ...
    protected function decodeJsonFromText(string $text): array
    {
        ...
        if ($trimmed === '') {
            throw new RuntimeException('Hermes completion content is empty.');
        }
        ...
        if (! is_array($decoded)) {
            throw new RuntimeException('Hermes completion content is not valid JSON.');
        }
    ...
    protected function validateTranslatedDocumentLanguage(array $translatedDocument, string $targetLang): void
    {
        ...
        foreach ($translatedDocument as $key => $value) {
            ...
            throw new RuntimeException(sprintf(
                'Hermes translated_document key [%s] contains Chinese characters for English target output.',
                $key,
            ));
        }
    }
